...
namespace NeroWeNeed.Terrain
{
    ...
    public sealed class TerrainNoiseChunkGenerationSystem : SystemBase
    {
        ...
        internal unsafe struct ChunkGeneration : IJobParallelFor
        {
            ...
            public NativeArray<MarchingCubeChunk> chunkData;
            [NativeDisableParallelForRestriction]
            public NativeHashMap<int2, MarchingCubeChunk> chunkCache;
            ...
            public void Execute(int index)
            {
                var targetChunkLocation = chunkLocation[index];
                if (!chunkCache.TryGetValue(targetChunkLocation, out var targetChunk))
                {
                    int3 chunkPos = math.int3(targetChunkLocation.x * MarchingCubes.ChunkHorizontalSpanInCells, 0, targetChunkLocation.y * MarchingCubes.ChunkHorizontalSpanInCells);
                    ...
                    chunkCache[targetChunkLocation] = targetChunk;
                }
        ...
        protected override void OnCreate()
        {
            ...
            chunkCache = new NativeHashMap<int2, MarchingCubeChunk>(8, Allocator.Persistent);
        ...
        protected unsafe override void OnUpdate()
        {
            ...
            if (lastVersion != version)
            {
                ...
                }.Schedule(chunks.Length, 1, Dependency);
