...
Shader "NeroWeNeed/Basic Terrain Material PBR"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings LitPassVertex(uint vertexId : SV_VertexID)
            {
                ...
                VertexPositionInputs vertexInput = GetVertexPositionInputs(Vertices[vertexId]);

                // Similar to VertexPositionInputs, VertexNormalInputs will contain normal, tangent and bitangent
                // in world space. If not used it will be stripped.
                half3 normal = Normals[vertexId];
                float3 v1 = cross(normal,float3(0,0,1));
                float3 v2 = cross(normal,float3(0,1,0));
                VertexNormalInputs vertexNormalInput = GetVertexNormalInputs(normal,length(v1) > length(v2) ? float4(v1,-1) : float4(v2,1));
                //VertexNormalInputs vertexNormalInput = GetVertexNormalInputs(Normals[vertexId], float4(TransformObjectToWorldDir(normalize(length(v1) > length(v2) ? v1 : v2)),1));
